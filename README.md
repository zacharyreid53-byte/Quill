...
- **`index.html`** — the cloud version. Real accounts (email + password via Firebase Authentication), quotes stored in the Firebase Realtime Database, so everything saves automatically and follows you to any device. Sharing sends a quote straight into another person's collection, wherever they are.
